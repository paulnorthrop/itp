...
We use three examples from Section 3 of [Oliveira and Takahashi (2021)](https://doi.org/10.1145/3423597) to illustrate the use of the `itp` function.  Each of these functions has a root in the interval $(-1, 1)$.
...
The Lambert function $f(x) = xe^x - 1$ is continuous.

```{r lambert, echo = FALSE, fig.align='center', fig.width=5, fig.height=3}
...
The staircase function $f(x) = \lceil 10 x - 1 \rceil + 1/2$ is discontinuous.

```{r staircase, echo = FALSE, fig.align='center', fig.width=5, fig.height=3}
oldpar <- par(mar = c(4, 4, 1, 1))
...
The Warsaw function $f(x) = I(x > -1)\left(1 + \sin\left(\frac{1}{1+x}\right)\right)-1$ has multiple roots.

```{r warsaw, echo = FALSE, fig.align='center', fig.width=5, fig.height=3}
oldpar <- par(mar = c(4, 4, 1, 1))
