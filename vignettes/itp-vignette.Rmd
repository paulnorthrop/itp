...
The `itp` function implements the ITP method to find a root $x^*$ of the function $f: \mathbb{R} \rightarrow \mathbb{R}$ in the interval $[a, b]$, where $f(a)f(b) < 0$. If $f$ is continuous then $f(x^*) = 0$. If $f$ is discontinuous then $x^*$ may be an estimate of the point at which a discontinuity occurs, that is, $f(x^* - \delta)f(x^* + \delta) \leq 0$, where $0 \leq \delta \leq \epsilon$ for some tolerance value $\epsilon$.
...
The function `itp` has an argument `fsign` that can be used to control the sign of the function at the estimated root $x^*$. This may be unnecessary for most problems, but this is included because I had a problem where I needed to avoid the function having a negative value at the estimated root. We return to the trigonometric and staircase examples above to illustrate this.  We set `fsign = "ge"` to force $f(x^*) \geq 0$ and `fsign = "le"` to force $f(x^*) \leq 0$.
